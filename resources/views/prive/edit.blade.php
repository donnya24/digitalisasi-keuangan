...
@section('title', 'Edit Prive')
@section('page-title', 'Edit Prive')

@section('content')
<div class="max-w-2xl mx-auto px-4">
    <div class="bg-white rounded-xl shadow-sm p-5">
        <form method="POST" action="{{ route('prive.update', $prive->id) }}" class="space-y-5" id="priveForm">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" 
                           name="amount" 
                           id="amount"
                           value="{{ old('amount', number_format($prive->amount, 0, ',', '.')) }}"
                           placeholder="0"
            ...
            <div>
                ...
                <input type="text" 
                       name="description" 
                       id="description"
                       value="{{ old('description', $prive->description) }}"
                       required
            ...
            <div>
                ...
                <input type="date" 
                       name="prive_date" 
                       id="prive_date"
                       value="{{ old('prive_date', $prive->prive_date) }}"
                       required
            ...
            <div>
                ...
                <select name="purpose" id="purpose"
                    ...
                    <option value="kebutuhan pribadi" {{ old('purpose', $prive->purpose) == 'kebutuhan pribadi' ? 'selected' : '' }}>Kebutuhan Pribadi</option>
                    <option value="belanja bulanan" {{ old('purpose', $prive->purpose) == 'belanja bulanan' ? 'selected' : '' }}>Belanja Bulanan</option>
                    <option value="pendidikan" {{ old('purpose', $prive->purpose) == 'pendidikan' ? 'selected' : '' }}>Pendidikan</option>
                    <option value="kesehatan" {{ old('purpose', $prive->purpose) == 'kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                    <option value="transportasi" {{ old('purpose', $prive->purpose) == 'transportasi' ? 'selected' : '' }}>Transportasi</option>
                    <option value="hiburan" {{ old('purpose', $prive->purpose) == 'hiburan' ? 'selected' : '' }}>Hiburan</option>
                </select>
                @error('purpose')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            ...
            <!-- Status (Readonly) -->
            @if($prive->is_approved)
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                <div class="px-4 py-3 bg-gray-50 rounded-lg text-sm">
            ...
            <div class="flex flex-col gap-2 pt-4">
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i> Update Prive
                </button>
